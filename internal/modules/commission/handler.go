...
func (h *Handler) GetCommissionRate(c *gin.Context) {
	httpx.Success(c, 200, "OK", gin.H{"rate": h.Service.GetCommissionRate()}, nil)
}
...
func (h *Handler) UpdateCommissionRate(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		httpx.Error(c, 400, "Invalid request body — rate must be a fraction between 0 and 1, e.g. 0.20")
		return
	}
	if err := h.Service.SetCommissionRate(req.Rate); err != nil {
		httpx.Error(c, 500, "Failed to update commission rate")
		return
	}
	httpx.Success(c, 200, "Commission rate updated", gin.H{"rate": req.Rate}, nil)
}
